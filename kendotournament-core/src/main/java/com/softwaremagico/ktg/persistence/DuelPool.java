...
public class DuelPool extends TournamentDependentPool<Duel> {
	...
	public List<Duel> get(RegisteredPerson competitor, boolean team1) throws SQLException {
		...
		for (Duel duel : allDuels) {
			if (team1) {
				if (duel.getFight().getTeam1().getMember(duel.getOrder(), duel.getFight().getIndex())
						.equals(competitor)) {
					results.add(duel);
				}
			} else {
				if (duel.getFight().getTeam2().getMember(duel.getOrder(), duel.getFight().getIndex())
						.equals(competitor)) {
					results.add(duel);
				}
			}
	...
	/**
	 * Obtain the duels for a fight. If the fight is new, then create the new duels and enqueue it to store into the
	 * database.
	 *
	 * @param fight
	 * @return
	...
	public List<Duel> get(Tournament tournament, RegisteredPerson competitor) throws SQLException {
		...
		List<Duel> results = new ArrayList<>();
		for (Duel duel : allDuels) {
			if (duel.getFight().getTeam1().isMember(competitor) || duel.getFight().getTeam2().isMember(competitor)) {
				results.add(duel);
			}
		}
		return results;
	...
	private static void clearDuelsPerFight() {
		duelsPerFight = new HashMap<>();
	}

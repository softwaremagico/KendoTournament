...
public class Media {
    ...
    public static BufferedImage getImageFitted(String imagePath, JPanel parent) {
        ...
        try {
            ...
            BufferedImage scaled = resizeImage(input, parent.getWidth(), parent.getHeight(), BufferedImage.TYPE_INT_ARGB);
            return scaled;
    ...
    private static BufferedImage resizeImage(BufferedImage originalImage, int panelWidth, int panelHeight, int type) {
        ...
        //System.out.println(originalImage.getWidth() + " " + originalImage.getHeight() + "/" + resizedImage.getWidth() + "  " + resizedImage.getHeight() + " / " + panelWidth + " " + panelHeight);

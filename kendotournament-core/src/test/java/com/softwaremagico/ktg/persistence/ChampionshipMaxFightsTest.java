...
@Test(groups = { "championshipMaxFightsTest" }, dependsOnGroups = { "populateDatabase" })
public class ChampionshipMaxFightsTest {

	private static final Integer MEMBERS = 3;
	private static final Integer TEAMS_PER_GROUP = 6;
	private static final Integer GROUPS = 2;
	public static final String TOURNAMENT_NAME = "championshipMaxFightsTest";
	...
	public void createFights() throws SQLException, PersonalizedFightsException {
		FightPool.getInstance().add(tournament, TournamentManagerFactory.getManager(tournament).createSortedFights(true, 0));
		Assert.assertTrue(FightPool.getInstance().get(tournament).size() == GROUPS * ((TEAMS_PER_GROUP * (TEAMS_PER_GROUP - 1)) / 2));
	}
	...
	public void solveFirstLevel() throws SQLException {
		...
		Assert.assertEquals(ranking1.getTeam(0), TeamPool.getInstance().get(tournament, "Team01"));
		Assert.assertEquals(ranking1.getTeam(1), TeamPool.getInstance().get(tournament, "Team02"));
		...
		Assert.assertEquals(ranking2.getTeam(0), TeamPool.getInstance().get(tournament, "Team07"));
		Assert.assertEquals(ranking2.getTeam(1), TeamPool.getInstance().get(tournament, "Team08"));
	}
	...
	public void solveSecondLevel() throws SQLException, PersonalizedFightsException {
		...
		Assert.assertTrue(group1.getTeams().contains(TeamPool.getInstance().get(tournament, "Team08")));
		TGroup group2 = TournamentManagerFactory.getManager(tournament).getLevel(1).getGroups().get(1);
		Assert.assertTrue(group2.getTeams().contains(TeamPool.getInstance().get(tournament, "Team07")));
		...
		Assert.assertEquals(ranking1.getTeam(0), TeamPool.getInstance().get(tournament, "Team01"));
		Ranking ranking2 = new Ranking(group2.getFights());
		Assert.assertEquals(ranking2.getTeam(0), TeamPool.getInstance().get(tournament, "Team02"));
	...
	public void solveThirdLevel() throws SQLException, PersonalizedFightsException {
		...
		Ranking ranking1 = new Ranking(group1.getFights());
		Assert.assertEquals(ranking1.getTeam(0), TeamPool.getInstance().get(tournament, "Team02"));
	}
}

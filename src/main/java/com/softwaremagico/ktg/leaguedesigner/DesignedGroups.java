...
public class DesignedGroups implements Serializable {
    ...
    private List<Integer> levels = new ArrayList<Integer>();  //"Pointer" to the designedGroup to point fights of the next level.
     ...
     */
    private void fillGroupWithWinnersPreviousLevel(int groupIndex, List<Fight> fights, boolean resolvDraw) {
            ...
            for (int i = 0; i < groups.size(); i++) {
                for (int k = 0; k < groups.get(i).returnMaxNumberOfWinners(); k++) {
                    ...
                    if (destination == returnPositionOfGroupInItsLevel(groupIndex)) {
                        ...
                        //  System.out.println(previousDg.getTeamInOrderOfScore(k, fights, true).returnName());
                    }
                }
                //System.out.println("----------");
            }
     ...
     * to continue.
     */
    public int firstLevelNotFinished(List<Fight> fights) {
        ...
        try {
            ...
            if (fights.isEmpty()) {
                ...
                } //All fights finished. We assume that a level is completed.
    ...
    public boolean isLevelFinished(List<Fight> fights, int level) {
    ...
    public List<Fight> nextLevel(List<Fight> fights, int fightArea, Tournament championship) {
        ...
        try {
            ...
            if (nextLevel >= 0 && nextLevel < returnNumberOfLevels()) {
                ...
                if (KendoTournamentGenerator.getInstance().fights.areAllOver() && MessageManager.question("nextLevel", "Warning!", language)) {
                    ...
                    for (int i = start; i < end; i++) {
                        fillGroupWithWinnersPreviousLevel(i, KendoTournamentGenerator.getInstance().fights.getFights(), true);
                    }
                    //set the team members order in the new level.
                    //updateOrderTeamsOfLevel(nextLevel);
                } else {
                    //Only one arena is finished: show message for waiting all fights are over.
                    if ((arena = KendoTournamentGenerator.getInstance().fights.allArenasAreOver()) != -1) {
                ...
                if (KendoTournamentGenerator.getInstance().fights.areAllOver() && KendoTournamentGenerator.getInstance().fights.size() > 0) {
        ...
        update();
        return generateLevelFights(nextLevel);
    }
     ...
     * @return
     */
    public List<Fight> generateLevelFights(int level) {
        List<Fight> fights = new ArrayList<Fight>();
        List<DesignedGroup> groups = returnGroupsOfLevel(level);

        boolean answer = false;
        int arena = 0;
        //User must distribute the groups of level 0 in the different fights areas. 
    ...
    private int obtainMaxLevel(List<Fight> fights) {
        int max = 0;
        for (int i = 0; i < fights.size(); i++) {
    ...
    public int getGroupOfFight(List<Fight> fights, int fightIndex) {
    ...
    public List<Fight> getFightsOfLevel(List<Fight> fights, int level) {
     ...
     */
    public void refillDesigner(List<Fight> tmp_fights) {
        ...
        //Fill Inner Levels
        updateInnerLevels();
        for (int i = 1; i < returnNumberOfLevels(); i++) {
            if (isLevelFinished(tmp_fights, i - 1)) {
                List<Integer> groupsOfLevel = returnIndexOfGroupsOfLevel(i);
                for (int j = 0; j < groupsOfLevel.size(); j++) {
                    fillGroupWithWinnersPreviousLevel(groupsOfLevel.get(j), KendoTournamentGenerator.getInstance().fights.getFights(), false);
                }
            } else {
                break;
            }
    ...
    private void refillLevel(List<Fight> tmp_fights, int level) {
        ...
        for (int i = 0; i < fights.size(); i++) {
            ...
            if (isTeamIncludedInList(teamsOfGroup, fights.get(i).team1)) {
                ...
                //If no team exist in this group, means that we find the fights of a new group.
     ...
     * @param level
     */
    private void completeGroupsWithOneTeam(List<Fight> tmp_fights, int level) {

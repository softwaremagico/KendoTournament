...
public class BlackBoardPanel extends javax.swing.JPanel {
    ...
    public void updateBlackBoard(String championshipName, boolean refill) {
        ...
        if (!refill) {
            ...
            KendoTournamentGenerator.getInstance().fights.getFightsFromDatabase(championshipName);
            ...
            List<Fight> fights = KendoTournamentGenerator.getInstance().database.searchFightsByTournamentName(KendoTournamentGenerator.getInstance().getLastSelectedTournament());
                ...
                for (int k = 0; k < designedGroupsOfLevel.get(j).returnMaxNumberOfWinners(); k++) {
                    if (designedGroupsOfNextLevel.size() > 1) {
                        if (!KendoTournamentGenerator.getInstance().designedGroups.mode.equals("manual") || i > 0) {
                            //    destination = (int) KendoTournamentGenerator.getInstance().fights.obtainPositonOfOneWinnerInTree(j, designedGroupsOfLevel.size()) / 2;

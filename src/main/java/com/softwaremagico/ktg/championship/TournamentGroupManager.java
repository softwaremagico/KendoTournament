...
public class TournamentGroupManager implements Serializable {
	 ...
	 * 
	 * @param group
	 *            group to complete relative to the level.
	 */
	...
	/*
	 * public Integer getIndexLastGroupUsed() { int last = 0; Integer level = getIndexLastLevelUsed(); if (level != null) { List<TournamentGroup>
	 * tournamentGroups = levels.get(level).getGroups(); for (int i = 0; i < tournamentGroups.size(); i++) { if (tournamentGroups.get(i).areFightsOver()) { last
	 * = i; } } } return last; }
	 */
	/**
	 * ********************************************
	 ...
	 */
	/*
	 * private TournamentGroup getLastGroupOfLevel(Integer level) { if (level >= 0 && level < levels.size()) { return levels.get(level).getLastGroupOfLevel(); }
	 * else { return null; } }
	 */
	...
	/**
	 * Create a new level depending of the number of level and the championship mode. 
	 * @param levelToCreate
	 * @return
	 */
	...
	public void convertLevelsToMode(List<LevelGroups> oldLevels) {
		levels = new ArrayList<>();
		for (int i = 0; i < oldLevels.size(); i++) {
			levels.add(createNextLevel(i));
			for(TournamentGroup group : oldLevels.get(i).getGroups()){
				levels.get(i).addGroup(group, group.isSelected());
			}
		}
	}
	...
	/*
	 * private int calculateNumberOfLevels() { return (int) (Math.log(getSizeOfLevel(0)) / Math.log(2)) + default_max_winners; }
	 */
	 ...
	 */
	/**
	 * The default arena of the groups into level zero is distributed proportionally.
	 */
	 ...
	 */
	/*
	 * private void completeGroupsWithOneTeam(ArrayList<Fight> tmp_fights, int level) { List<Team> winners = new ArrayList<>(); if (level > 0 &&
	 * (getFightsOfLevel(tmp_fights, level - 1).size() > 0) && (getFightsOfLevel(tmp_fights, level).size() > 0)) { //obtain all winners of previous level.
	 * List<TournamentGroup> groupsPrev = returnGroupsOfLevel(level - 1); for (int i = 0; i < groupsPrev.size(); i++) { if
	 * (groupsPrev.get(i).areFightsOverOrNull(tmp_fights)) { winners.addAll(groupsPrev.get(i).getWinners()); } }
	 * 
	 * //Delete the winners inserted in a group of this level. List<TournamentGroup> groups = returnGroupsOfLevel(level); for (int i = 0; i < groups.size();
	 * i++) { for (int j = 0; j < groups.get(i).teams.size(); j++) { for (int k = 0; k < winners.size(); k++) { if
	 * (winners.get(k).returnName().equals(groups.get(i).teams.get(j).returnName())) { winners.remove(k); k--; } } } }
	 * 
	 * //Add the winner without group to the last one. TournamentGroup lastg = groups.get(groups.size() - 1); if (lastg != null) { for (int i = 0; i <
	 ...
	 */
	/**
	 * Inherit from SerialParent this class allow to save the Computer structure in a file.
	 * 
	 * @see SerialParent
	 */

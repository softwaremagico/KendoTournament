...
public class PasswordConfirmator extends KFrame {
	private static final long serialVersionUID = 2025016043586443325L;
	private static final Integer MAX_PASS_LENGTH = 15;
	private JPasswordField passwordField;
	private PasswordConfirmator reference;
	private FightPanel parent;

	public PasswordConfirmator(FightPanel parent) {
		defineWindow(300, 145);
		this.parent = parent;
		setResizable(false);
		setVisible(true);
		// setAlwaysOnTop(true);
		...
		AcceptButton okButton = new AcceptButton() {
			...
			public void acceptAction() {
				if (String.copyValueOf(passwordField.getPassword()).equals(
						String.copyValueOf(KendoTournamentGenerator.getBlockingString()))) {
					KendoTournamentGenerator.setBlockingString(null);
					parent.unblockMonitor();
					reference.dispose();
				} else {
					AlertManager.errorMessage(PasswordConfirmator.class.getName(), "passwordDoesNotMatch", "Password");

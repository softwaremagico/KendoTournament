...
public class FightPanel extends KFrame {
	...
	private JMenuItem showTreeMenuItem, groupScoreMenuItem, globalScoreMenuItem, deleteFightMenuItem, addFightMenuItem,
	...
	private JMenu windowMenu() {
		...
		// Ctrl + S for save.
		updateDatabase.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_S,
	...
	public void updateSelectedTournament() {
		...
		if (getSelectedTournament() != null
				&& (getSelectedTournament().getType().equals(TournamentType.CHAMPIONSHIP)
						|| getSelectedTournament().getType().equals(TournamentType.LEAGUE_TREE) || getSelectedTournament()
	...
	/**
	 * If there are more than one team with the max punctuation, ask for a winner to the user.
	 * 
	...
	/**
	 * Real next button action. Creates next fights, ask for fights in other computers and show information messages to
	 * the user.
	 */
					...
					if (FightPool.getInstance().areAllOver(getSelectedTournament())) {
						...
						try {
							...
							if (newFights != null && newFights.size() > 0) {
								...
								// message.
								if (ranking != null) {
									JDialog winnerMessage = AlertManager.winnerMessage(this.getClass().getName(),
											"leagueFinished", "!!!!!!", ranking.getTeamsRanking().get(0).getName());
						...
						if (FightPool.getInstance().areAllOver(getSelectedTournament(), getSelectedFightArea())) {
							...
							waitingArenaDialog = AlertManager.createWaitingArenasMessage(getSelectedTournament(),
									getSelectedFightArea());
	...
	/**
	 * Database must be updated if different arenas (with different computers) are used.
	 */
	...
	/**
	 * Swing worker for exchange data between computers and show a waiting message without freeze the GUI.
	 */

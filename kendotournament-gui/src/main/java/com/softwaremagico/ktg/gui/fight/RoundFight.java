...
public class RoundFight extends JPanel {
	...
	protected RoundFight(Tournament tournament, Fight f, boolean selected, boolean menu, int fight_number,
			boolean invertedTeam, boolean invertedColor) {
	...
	private void fillCurrentFightPanel(Fight f, boolean selected, boolean menu, int fight_number, boolean invertedTeam,
			boolean invertedColor) {
		fight = f;
		removeAll();
		...
		if (!invertedTeam) {
			tf1 = new TeamFight(tournament, this, f.getTeam1(), f, true, selected, menu, fight_number, fight_total,
					invertedColor);
		} else {
			tf1 = new TeamFight(tournament, this, f.getTeam2(), f, true, selected, menu, fight_number, fight_total,
					invertedColor);
		}

		if (!invertedTeam) {
			tf2 = new TeamFight(tournament, this, f.getTeam2(), f, false, selected, menu, fight_number, fight_total,
					invertedColor);
		} else {
			tf2 = new TeamFight(tournament, this, f.getTeam1(), f, false, selected, menu, fight_number, fight_total,
					invertedColor);
		}
	...
	private class DrawPanel extends KPanel {
		...
		private void updateDrawPanel(PanelBackground panel, int winner, int over, boolean nextStarted, int index) {
			if ((winner == 0 && over != 2) || // fight over and the duel is
												// draw.
					(winner == 0 && nextStarted) || // fight not over, but next
													// duel started
					(drawsAnnoted.get(index) && winner == 0)) { // Set by the
																// user.

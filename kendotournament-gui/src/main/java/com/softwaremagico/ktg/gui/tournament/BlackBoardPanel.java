...
public class BlackBoardPanel extends javax.swing.JPanel {
	...
	private void paintDesignedGroups() {
		...
		for (int i = 0; i < TournamentManagerFactory.getManager(tournament).getNumberOfLevels(); i++) {
			...
			if (i < TournamentManagerFactory.getManager(tournament).getNumberOfLevels() - 2) {
				s = new Separator(trans.getTranslatedText("Round") + " "
						+ (TournamentManagerFactory.getManager(tournament).getNumberOfLevels() - i));
		...
		for (int level = 0; level < TournamentManagerFactory.getManager(tournament).getNumberOfLevels(); level++) {
			for (int groupIndex = 0; groupIndex < TournamentManagerFactory.getManager(tournament).getGroups(level)
					.size(); groupIndex++) {
				try {
					...
					for (int j = 0; j < designedGroupsFromLevel.size(); j++) {
						for (int winners = 0; winners < designedGroupsFromLevel.get(j).getMaxNumberOfWinners(); winners++) {
							if (designedGroupsToLevel.size() > 0) {
								destination = TournamentManagerFactory
										.getManager(tournament)
										.getLevel(i)
										.getGroupIndexDestinationOfWinner(
												designedGroupsFromLevel.get(j).getTournamentGroup(), winners);
							} else { // Final group
								destination = null;
							}
							if (destination != null && destination < designedGroupsToLevel.size() && destination >= 0) {
								drawLink(g, designedGroupsFromLevel.get(j), designedGroupsToLevel.get(destination),
										winners, j, destination, j < designedGroupsFromLevel.size() / 2);
							}
						}
					}
				}
	...
	private void paintArrow(Graphics g, double x0, double y0, double x1, double y1) {
		...
		int xpoints[] = { (int) x1, (int) x1 - (int) ((1 - frac) * deltaX + frac / 3 * deltaY),
				// (int) x1 - (int) ((1 - frac / 2) * deltaX),
				(int) x1 - (int) ((1 - frac) * deltaX - frac / 3 * deltaY) };
		int ypoints[] = { (int) y1, (int) y1 - (int) ((1 - frac) * deltaY - frac / 3 * deltaX),
	...
	private void drawLink(Graphics g, TournamentGroupBox d1, TournamentGroupBox d2, int winner, int originNumber,
			int destinationNumber, boolean half) {
		...
		if (d1.getMaxNumberOfWinners() > 1) {
			// paintArrow(g, r1.x + r1.width, r1.y + ((r1.height /
			// (d1.getMaxNumberOfWinners() + 1)) * (winner + 1)), r2.x, r2.y +
			// r2.height / 2);
			if ((originNumber < destinationNumber) || (half && originNumber == destinationNumber)) {
				paintArrow(g, r1.x + r1.width, r1.y + ((r1.height / (d1.getMaxNumberOfWinners() + 1)) * (winner + 1)),
						r2.x, r2.y + r2.height / 2 - 4);
			} else {
				paintArrow(g, r1.x + r1.width, r1.y + ((r1.height / (d1.getMaxNumberOfWinners() + 1)) * (winner + 1)),
						r2.x, r2.y + r2.height / 2 + 4);
			}
		} else {
			// paintArrow(g, r1.x + r1.width, r1.y + r1.height / 2, r2.x, r2.y +
			// r2.height / 2);
			if (originNumber % 2 == 0) {
	...
	public Integer getSelectedBoxIndex() {
		if (selectedGroupIndex != null && grpsBox != null && grpsBox.get(0) != null && selectedGroupIndex >= 0
				&& selectedGroupIndex < grpsBox.get(0).size()) {
	...
	void clicked(java.awt.event.MouseEvent e, TournamentGroupBox group) {
		...
		if (group.getTournamentGroup().getLevel() == 0) {
			if (e.getClickCount() == 2) {
				...
				// Avoid to run the one-click functions when performing a
				// doubleclick.

...
public class SummaryPDF extends ParentList {
	...
	protected boolean showNotFinishedFights = true; // If true, only show not
													// finished fights, if false
													// only show
	// finished fights.
	protected boolean showAll = true; // If true, show finished and not finished
										// fights;
	protected boolean divideByShiaijo = false; // If true, each shiaijo in
												// different page.
			...
			if (tournamentManager.getLevel(l) == null || tournamentManager.getLevel(l).hasFightsAssigned()) {
				...
				for (int i = 0; i < groups.size(); i++) {
					...
					if (useOnlyShiaijo < 0 || groups.get(i).getFightArea().equals(useOnlyShiaijo)) {
						mainTable.addCell(getEmptyRow());
						StringBuilder header = new StringBuilder(trans.getTranslatedText("GroupString") + " " + (i + 1));
						if (useOnlyShiaijo < 0) {
							header.append(" (" + trans.getTranslatedText("FightAreaNoDots") + " " + Tournament.getFightAreaName(groups.get(i).getFightArea())
									+ ")");
						}
	...
	public void createHeaderRow(Document document, PdfPTable mainTable, float width, float height, PdfWriter writer, String font, int fontSize) {
		...
		if (useOnlyShiaijo >= 0) {
			header += " (" + trans.getTranslatedText("FightAreaNoDots") + " " + Tournament.getFightAreaName(useOnlyShiaijo) + ")";
		}

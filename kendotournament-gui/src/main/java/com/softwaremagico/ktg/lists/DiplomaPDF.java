	...
	public class ThreadDiploma extends Thread {
		...
		public boolean convertPDF() {
			...
			try {
				...
				AlertManager.translatedMessage(this.getClass().getName(), "diplomaOK", "PDF",
		...
		private void pagePDF(Document document, PdfWriter writer, String font) throws Exception {
			pageTable(document, document.getPageSize().getWidth(), document.getPageSize().getHeight(), writer, font,
					fontSize);
		}

		public void pageTable(Document document, float width, float height, PdfWriter writer, String font, int fontSize)
		...
		private void diplomaTable(Document document, PdfWriter writer, RegisteredPerson competitor, String font,
				int fontSize) throws BadElementException, DocumentException, MalformedURLException, IOException {
			PdfPCell cell;
			Paragraph p;
			...
			mainTable.writeSelectedRows(0, -1, 0, document.getPageSize().getHeight() * nameposition + 27,
		...
		private void statisticsTable(Document document, PdfWriter writer, RegisteredPerson competitor, String font,
				int fontSize) {
			try {
				PdfPCell cell;
				...
				// General hits.
				Image image = com.itextpdf.text.Image.getInstance(
						createPieChart(createGeneralHitsDataset(), transl.getTranslatedText("TitleHits")), null, false);
			...
			for (int i = startValue; i < endValue; i++) {
				...
				if (tournament == null) {
					c = (i + 1) + " - " + teamTopTen.get(i).getTeam().getName() + " ("
							+ teamTopTen.get(i).getTournament().getName() + ")";
				} else {
					c = (i + 1) + " - " + teamTopTen.get(i).getTeam().getName();
				}
